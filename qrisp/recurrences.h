...
inline score_t EncodeTraceback(const int& i, const int& j,
                               const int& rna_size) {
  CHECK(i >= 0);
  CHECK(j >= 0);
  CHECK(rna_size > 0);
...
inline std::tuple<int, int> DecodeTraceback(const score_t& s,
                                            const int& rna_size) {
  CHECK(s >= 0.0);
  CHECK(rna_size > 0);
  const int val = (int)s;
...
class LossFunctor {
 public:
  ...
  std::tuple<score_t, score_t> GetLossTerms(const int i, const int j);

  // This method calculates the loss terms for pairs that occur withing the
  // half-open interval [i,j).
  inline double PairingLoss(uint32_t i, uint32_t j) {
